...
def remove_parentheses(expression):
    ...
    reduce_re1 = re.compile(r'(\d+ \+ \d+)')
    reduce_re2 = re.compile(r'(\d+ \* \d+)')
    match = parentheses_re.search(expression)
    while match:
        reduced = match[0]
        to_reduce = reduce_re1.search(reduced)
        while to_reduce:
            sub_result = eval(to_reduce[1])
            reduced = reduced.replace(to_reduce[1], f'{sub_result}', 1)
            to_reduce = reduce_re1.search(reduced)
        to_reduce = reduce_re2.search(reduced)
        while to_reduce:
            sub_result = eval(to_reduce[1])
            reduced = reduced.replace(to_reduce[1], f'{sub_result}', 1)
            to_reduce = reduce_re2.search(reduced)
